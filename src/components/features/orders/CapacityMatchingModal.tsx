...
const CapacityMatchingModal: React.FC<CapacityMatchingModalProps> = ({ isOpen, onClose, orderId }) => {
  ...
  const transportModes = [
    ...
    { id: 'road', icon: <Truck size={32} />, label: '陆运-卡车', sub: '国内干线', price: '¥20/kg', cap: '30k Cap', status: '已满', color: 'text-slate-500', bg: 'bg-slate-800/40', disabled: true },
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-[#0B0F19] w-full max-w-4xl rounded-[40px] border border-slate-800 shadow-[0_32px_128px_-16px_rgba(0,0,0,1)] overflow-hidden flex flex-col transform animate-in zoom-in-95 duration-300">
        ...
        <div className="px-10 py-8 border-b border-slate-800/50 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-black text-white tracking-tight">选择运输工具</h2>
          </div>
          <button 
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500 hover:text-white transition-all active:scale-95"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-10 space-y-8">
          {/* Order Detail Card */}
          <div className="bg-[#111827] border border-slate-800 rounded-3xl p-6 flex justify-between items-center relative group">
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-black text-white tabular-nums tracking-tighter">{orderId}</span>
              </div>
              ...
              <div className="flex items-center gap-12">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-5">
                    <span className="text-sm font-bold text-slate-200">{origin}</span>
                ...
                <div className="flex gap-8 border-l border-slate-800 pl-8">
                  <div className="space-y-1">
                    <div className="text-[10px] text-slate-600 font-black uppercase tracking-widest">总计重量</div>
                    <div className="text-sm font-black text-white">150 kg</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-[10px] text-slate-600 font-black uppercase tracking-widest">总计体积</div>
                    <div className="text-sm font-black text-white">5 m³</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Map Snippet */}
            <div className="w-48 h-28 bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden relative shadow-inner">
               <img 
               ...
               <div className="absolute bottom-2 right-2 px-1.5 py-0.5 bg-black/60 rounded text-[8px] text-white font-bold uppercase tracking-widest">LIVE MAPPING</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-6">
            {transportModes.map((mode) => (
              <button
                ...
                className={`flex flex-col h-72 border transition-all rounded-3xl relative p-6 text-left group ${
              ...
              >
                ...
                <div className={`absolute top-4 right-4 w-6 h-6 rounded-full border flex items-center justify-center transition-all ${
                  selectedMode === mode.id ? 'bg-blue-600 border-blue-400' : 'border-slate-700'
                }`}>
                  {selectedMode === mode.id && <Check size={14} className="text-white" />}
                </div>
                ...
                <div className="space-y-1.5 mt-auto">
                  ...
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{mode.sub}</div>
                  
                  <div className="flex justify-between items-center pt-2 mt-2 border-t border-slate-800/50">
                    <span className="text-[10px] font-black text-slate-300">{mode.price} | {mode.cap}</span>
                    <span className={`text-[10px] font-black uppercase flex items-center gap-1.5 ${
                      ...
                      <div className={`w-1 h-1 rounded-full ${
                        ...
                        mode.status === '部分占用' ? 'bg-orange-500' : 'bg-slate-600'
                      }`} />
                      {mode.status}
                    </span>
                  </div>
          ...
          <div className="bg-[#111827] border border-slate-800 rounded-3xl p-8 space-y-6">
             <div className="flex items-center justify-between border-b border-slate-800/50 pb-6">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-xs font-bold text-slate-400">碳排放等级</span>
                </div>
                <span className="text-sm font-black text-white">A级 (低排放 - 航空最优)</span>
             </div>
             <div className="flex items-center justify-between border-b border-slate-800/50 pb-6">
                <div className="flex items-center gap-4">
                  <Clock size={18} className="text-blue-400" />
                  <span className="text-xs font-bold text-slate-400">预估时效</span>
                </div>
                <span className="text-sm font-black text-white italic tracking-tight">18-24 小时 (极快)</span>
             </div>
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <ShieldCheck size={18} className="text-blue-400" />
                  <span className="text-xs font-bold text-slate-400">服务可靠性</span>
                </div>
                <span className="text-sm font-black text-white">99.9% 准时率</span>
             </div>
          </div>
        </div>
        ...
        <div className="px-10 py-8 bg-slate-950/40 border-t border-slate-800/50 flex justify-end gap-4">
          <button 
            onClick={onClose}
            className="px-10 py-4 bg-slate-900 border border-slate-800 text-slate-400 text-sm font-black rounded-2xl hover:bg-slate-800 transition-all uppercase tracking-[0.2em]"
          >
            取消
          </button>
          <button 
            onClick={onClose}
            className="px-12 py-4 bg-blue-600 text-white text-sm font-black rounded-2xl shadow-xl shadow-blue-600/30 hover:scale-[1.02] transition-all active:scale-95 uppercase tracking-[0.2em]"
          >
            确定方案
          </button>
        </div>
