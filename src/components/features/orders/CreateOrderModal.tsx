...
const CreateOrderModal: React.FC<CreateOrderModalProps> = ({ isOpen, onClose, onOrderCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 发送请求到后端创建订单
      const response = await fetch('http://81.71.129.36:8010/api/orders', {
        method: 'POST',
        headers: {
      ...
      <div className="bg-[#0B0F19] w-full max-w-xl rounded-[40px] border border-slate-800 shadow-[0_32px_128px_-16px_rgba(0,0,0,1)] overflow-hidden flex flex-col transform animate-in zoom-in-95 duration-300">
        ...
        <div className="px-10 py-8 border-b border-slate-800/50 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-white tracking-tight">创建新订单</h2>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Initialize New Logistics Task</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-[#111827] border border-slate-800 rounded-2xl pl-12 pr-4 py-4 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50 transition-all shadow-inner placeholder:text-slate-700 font-mono"
                />
              </div>
            </div>
