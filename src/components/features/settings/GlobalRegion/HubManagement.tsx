    ...
    <div className="bg-[#0B121E] border border-slate-800 rounded-[32px] p-8 shadow-2xl space-y-8 relative overflow-hidden">
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-black text-white uppercase tracking-widest">核心枢纽状态</h3>
        </div>
        <div className="text-[10px] text-slate-600 font-black uppercase tracking-widest animate-pulse">实时健康度检测中...</div>
      </div>

      <div className="space-y-4 relative z-10">
        {hubs.map((hub, idx) => (
          <div key={idx} className="bg-slate-950/40 border border-slate-800 rounded-2xl p-5 group hover:border-cyan-500/30 transition-all">
            <div className="flex justify-between items-start mb-4">
              <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-slate-300 transition-colors">{hub.name}</div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-[10px] font-black">
                <span className="text-slate-400">实时负载强度</span>
                <span className="text-white italic">{hub.load}%</span>
              </div>
