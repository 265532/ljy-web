...
const SyncStrategyCard: React.FC<SyncStrategyCardProps> = ({ activeStrategy, onSelect }) => {
  ...
  return (
    <div className="bg-[#0B121E] border border-slate-800 rounded-[32px] p-8 shadow-2xl h-full flex flex-col gap-10">
      <div className="flex items-center gap-3 text-blue-400">
        <div className="p-2.5 bg-blue-500/10 rounded-xl border border-blue-500/20">
          <ArrowRightLeft size={20} />
        </div>
        <h3 className="text-sm font-black text-white uppercase tracking-widest">同步频率与冲突策略</h3>
      </div>
      ...
      <div className="space-y-4 flex-1">
        {strategies.map((strat) => (
          <div 
            ...
            className={`p-6 rounded-[32px] border transition-all cursor-pointer relative group flex flex-col gap-3 ${
          ...
          >
            <div className="flex justify-between items-start">
               <h4 className={`text-sm font-black ${activeStrategy === strat.id ? 'text-white italic' : 'text-slate-400'} uppercase tracking-tight`}>
                 {strat.title}
               </h4>
               <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                 activeStrategy === strat.id ? 'border-blue-500 bg-blue-500 shadow-[0_0_10px_#3b82f6]' : 'border-slate-800'
               }`}>
                  {activeStrategy === strat.id && <CheckCircle2 size={14} className="text-white" />}
               </div>
            </div>
            <p className={`text-[11px] leading-relaxed ${activeStrategy === strat.id ? 'text-slate-300' : 'text-slate-600'} font-medium`}>
              {strat.desc}
            </p>
            {activeStrategy === strat.id && (
      ...
      <div className="mt-auto pt-8 border-t border-slate-800/50">
         <div className="bg-blue-500/5 border border-blue-500/20 rounded-[24px] p-6 flex gap-5 items-start group hover:bg-blue-500/10 transition-all cursor-default">
            ...
            <div className="space-y-2">
              <h4 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">数据吞吐预估</h4>
