...
const MonitoringStatus: React.FC = () => {
  return (
    <div className="bg-[#0B121E] border border-slate-800 rounded-[32px] p-8 shadow-2xl h-full flex flex-col gap-8">
      <div className="flex items-center gap-3 text-emerald-400">
        ...
        <h3 className="text-sm font-black text-white uppercase tracking-widest">实时监控态势</h3>
      </div>
      ...
      <div className="bg-red-500/5 border border-red-500/20 rounded-[24px] p-6 flex gap-4 items-center">
         ...
         <div className="space-y-1">
            <h4 className="text-[11px] font-black text-white uppercase tracking-widest">检测到 1 个异常项</h4>
  ...
  <div className="space-y-3 group">
    <div className="flex justify-between items-end">
      <div className="space-y-1">
        ...
        <div className="text-lg font-black text-white italic tracking-tighter tabular-nums">{current}</div>
      </div>
      <div className="text-right">
        <span className={`text-[10px] font-black uppercase tracking-widest ${status === 'Alert' ? 'text-red-500' : 'text-emerald-500'}`}>{status}</span>
        <div className="text-[9px] text-slate-600 font-bold uppercase mt-0.5">上限: {limit}</div>
      </div>
    </div>
