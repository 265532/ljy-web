...
import { AlertTriangle, Zap, Terminal } from 'lucide-react';
import TrainingLogModal from './Logs/TrainingLogModal';
  ...
  return (
    <div className="space-y-4 h-full">
      ...
      <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex gap-3">
        <div className="bg-red-500/20 p-2 rounded-lg h-fit"><AlertTriangle className="text-red-500" size={18} /></div>
        <div>
      ...
      <div className="bg-[#151B28] border border-cyan-500/30 rounded-xl p-5 space-y-4">
        <div className="flex items-center gap-3"><Zap className="text-cyan-400" size={20} fill="currentColor" /><h4 className="text-xs font-bold text-white">AI 优化已就绪</h4></div>
        <p className="text-[10px] text-slate-400 leading-relaxed">北美航线的新强化学习策略可减少 <span className="text-cyan-400 font-bold">2.3%</span> 的延迟。</p>
        <button className="w-full py-2 bg-cyan-500 text-black text-[11px] font-bold rounded-lg hover:bg-cyan-400 transition-colors shadow-[0_0_15px_rgba(6,182,212,0.3)]">应用策略</button>
      </div>
      ...
      <TrainingLogModal isOpen={isLogOpen} onClose={() => setIsLogOpen(false)} />
    </div>
  );
};
