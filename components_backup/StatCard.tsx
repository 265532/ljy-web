...
const StatCard: React.FC<StatCardProps> = ({ 
  ...
  const dummyData = Array.from({ length: 10 }, (_, i) => ({ value: Math.random() * 100 }));

  return (
    <div className="bg-[#151B28] rounded-xl p-5 border border-slate-800/50 hover:border-slate-700 transition-all group overflow-hidden relative">
      ...
      {type === 'line' && (
        <div className="absolute bottom-0 left-0 h-1 bg-cyan-500 w-2/3 shadow-[0_0_8px_rgba(6,182,212,0.8)]" />
      )}
      {type === 'bar' && (
        <div className="absolute bottom-0 left-0 flex gap-1 px-4 mb-2">
           <div className="h-1.5 w-4 bg-slate-800 rounded-full" />
           <div className="h-1.5 w-6 bg-amber-500 rounded-full shadow-[0_0_8px_rgba(245,158,11,0.5)]" />
           <div className="h-1.5 w-6 bg-amber-500 rounded-full shadow-[0_0_8px_rgba(245,158,11,0.5)]" />
           <div className="h-1.5 w-4 bg-slate-800 rounded-full" />
        </div>
      )}
