...
import { X, Leaf, TrendingDown, TrendingUp, Cpu, Plane, Ship, Train, FileText, ChevronRight } from 'lucide-react';
...
const CarbonMonitoringModal: React.FC<CarbonMonitoringModalProps> = ({ isOpen, onClose, orderId }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-[#0F172A] w-full max-w-xl rounded-[40px] border border-slate-800 shadow-[0_32px_128px_-16px_rgba(0,0,0,1)] overflow-hidden flex flex-col transform animate-in zoom-in-95 duration-300">
        ...
        <div className="px-8 py-8 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-baseline gap-3">
              <h2 className="text-xl font-black text-white tracking-tight">碳排放快览</h2>
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Carbon Quick View</span>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="px-8 pb-10 space-y-8">
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-7 bg-[#1E293B]/40 rounded-3xl p-6 border border-slate-800/50 flex flex-col justify-between h-44">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-slate-200 uppercase tracking-widest">每周排放趋势</span>
                <span className="text-[9px] text-slate-500 font-bold">最近4周</span>
              </div>
            ...
            <div className="col-span-5 bg-emerald-500/5 border border-emerald-500/20 rounded-3xl p-6 flex flex-col gap-4">
              ...
              <p className="text-[13px] text-slate-200 leading-relaxed font-medium">
                建议切换至 <span className="text-white font-black">路线 A</span>，预计可降低单箱碳足迹 <span className="text-emerald-400 font-black">15%</span>。
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              <h3 className="text-xs font-black text-white uppercase tracking-widest">高排放订单 (TOP 3)</h3>
          ...
          {/* Footer Button and Link */}
          <div className="pt-2 flex flex-col gap-5 items-center">
            <button 
              onClick={handleGenerateReport}
              className="w-full py-5 bg-[#10B981] text-[#0F172A] text-sm font-black rounded-2xl shadow-xl shadow-emerald-500/20 hover:scale-[1.01] transition-all active:scale-95 flex items-center justify-center gap-3"
            >
              <FileText size={18} fill="currentColor" fillOpacity={0.4} /> 生成详细分析报告
            </button>
            <button className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] flex items-center gap-2 hover:text-emerald-400 transition-colors">
              查看完整看板分析 <ChevronRight size={14} />
            </button>
          </div>
        </div>
      </div>
...
const MetricCard = ({ label, value, unit, trend, sub, isPositiveTrend }: any) => (
  <div className="bg-[#1E293B]/40 border border-slate-800/50 rounded-3xl p-5 space-y-3">
...
const EmissionOrderItem = ({ id, route, val, mode, icon, color }: any) => (
  <div className="bg-[#111827] border border-slate-800 rounded-2xl p-4 flex items-center justify-between group hover:border-slate-700 transition-all">
    <div className="flex items-center gap-4">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${color}`}>
        {icon}
      </div>
      <div>
        <div className="text-xs font-black text-white italic">#{id}</div>
        <div className="text-[9px] text-slate-500 font-bold mt-0.5">{route}</div>
      </div>
    </div>
    <div className="text-right">
      <div className="text-sm font-black text-orange-400 tracking-tighter italic">{val}</div>
      <div className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mt-0.5">{mode}</div>
    </div>
  </div>
);
